...
# co2 emissions data
state_emissions_2017 <- read_excel("data/co2_emissions_profile_by_state.xlsx", skip = 4, n_max = 51,col_names = c("state", "commercial_tons", "electric_power_tons", "residential_tons",	"industrial_tons",	"transportation_tons", "total_tons", "blank", "commercial_pct",	"electric_power_pct",	"residential_pct",	"industrial_pct",	"transportation_pct" 
))
state_emissions_2017 <- state_emissions_2017[-9,]
state_emissions_2017$blank <- NULL

# population data
pop <- read_csv("data/nst-est2019-alldata.csv", col_types = cols_only(NAME = "c", POPESTIMATE2017 = "i"))
pop$state <- pop$NAME
pop$NAME <- NULL
...
# gdp data
gdp <- read_excel("data/gdp_by_state_2017_bea.xls", range = "B8:C58", col_names = c("state", "gdp"))

# lat long data points for each state
state_lat_long <- read_csv("data/world_country_and_usa_states_latitude_and_longitude_values.csv", 
...
avgprice_annual <- read_excel("data/avgprice_annual.xlsx", skip = 1)
...
annual_generation_state <- read_excel("data/annual_generation_state.xls", skip = 1)
...
# climate data
hdd <- read_excel("data/2017_statelevel_climate_data.xlsx", 
                  sheet = "HDD_plaintext")
cdd <- read_excel("data/2017_statelevel_climate_data.xlsx", 
                  sheet = "CDD_plaintext")
